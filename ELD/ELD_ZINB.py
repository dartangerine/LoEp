...
def calculate_binomial_pvalue(window_values1, bg_values1, window_values2, bg_values2):
    """
    使用窗口内均值和 box 内方差构建（ZINB-aware）负二项模型，
    并用 Wald 检验比较两条 track。

    ZINB 修正方式：
    - 从 box 估计零膨胀比例 pi
    - 使用有效样本量 n_eff = n * (1 - pi)
    - 仅修正 Var(log(mu))，不修正 mu 本身
    """
    try:
        # 窗口长度（每个 bin 为一个观测）
        ...
        # 均值为 0 或负，无法做 log，直接返回不显著
        if mu1 <= 0 or mu2 <= 0:
            return 1.0

        # ----------------------------
        # box 内方差估计（用于 dispersion）
        # ----------------------------
        if len(bg_values1) > 1:
