...
def calculate_binomial_pvalue(window_values1, bg_values1, window_values2, bg_values2):
    """
    使用窗口内均值和box内方差构建负二项分布，并用 Wald 检验比较两条 track

    输入:
        window_values1: np.array, 窗口内第一个 track 的每-bin 值
        bg_values1: np.array, 对应 box（背景）中每-bin 值（用于估计方差）
        window_values2, bg_values2: 同上，但用于第二个 track

    输出:
        p_value: 双侧 Wald 检验的 p-value（float）

    处理细节:
    - μ 使用窗口内每-bin 的平均值 (mean)
    - 使用 box 的样本方差估计 Var_bg
    - dispersion alpha = max(0, (Var_bg - mu) / mu^2)
    - Var(log(mu_hat)) ≈ (1/n_window) * (1/mu + alpha)
    - Wald z = (log(mu1) - log(mu2)) / sqrt(var1 + var2)
    - 返回双侧 p = 2 * sf(|z|)
    """
    try:
        # 窗口长度（每个bin为一个观测）
        ...
        # 计算每-bin 均值 μ（注意：窗口可能包含浮点值）
        mu1 = np.mean(window_values1)
        mu2 = np.mean(window_values2)

        # 如果均值为0或负，无法取对数，直接返回 p=1
        if mu1 <= 0 or mu2 <= 0:
            return 1.0

        # 背景方差估计（如果样本数小，使用 ddof=0 回退）
        if len(bg_values1) > 1:
        ...
        # 根据 Var = mu + alpha * mu^2 反解 alpha
        # 注意：这里用窗口的 mu 作为 mu 的估计，方差来自 box（背景）
        alpha1 = (var_bg1 - mu1) / (mu1 ** 2) if mu1 > 0 else 0.0
        alpha2 = (var_bg2 - mu2) / (mu2 ** 2) if mu2 > 0 else 0.0

        # 不允许负的 dispersion；若为负则退化为 Poisson (alpha=0)
...
def process_chunk(args):
    ...
    for i in range(start_idx, end_idx):
        ...
        for window_size in window_sizes:
            ...
            diff_value = calculate_binomial_pvalue(window_values1, bg_values1, window_values2, bg_values2)
